...
public class PuzzleManager : MonoBehaviour {
	...
	private TileProperties selected;
	...
	void Start () 
	{
		...
		//load prefs
		Debug.Log (PlayerPrefs.GetInt("LevelPuzzle"));
		CreateLevel(PlayerPrefs.GetInt("LevelPuzzle"));

	}

	void OnGUI ()
	{
		if(GUI.Button(new Rect(15, 15, backBtnTexture.width, backBtnTexture.height), backBtnTexture, new GUIStyle()))
		{
	...
	{
		// if level is complete, unlock the next puzzle
		// prefs
	...
	GameObject CreateTile (Vector2 pos, Texture2D tex, bool collider)
	{
		...
		sprite = Sprite.Create(tex, new Rect(0, 0, tileSize, tileSize), new Vector2(0,0), 100.0f);	
